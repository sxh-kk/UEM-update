...
## 2026-09-23 15:02 +08:00：Issue #3 的无权重阶段修复与诊断

- **阶段 A，固定协议/评估健壮性。** 新增 `config/egorecover_pilot_split_v1.json` 固定官方 val 工程 8/2/2 take，训练 seed 与采样 seed 分离；缓存和回放绑定 split、数据 spec、统计量和模型 SHA。SMPL-X GT 资产审计现在拒绝原始 SMPL 参数、标注关节、生成几何或审计结果中的 NaN/Inf，报告使用严格 JSON。故障区间取数据 operations 的真实起止时刻，连续/重叠区间合并，离散区间明确拒绝；新评估保存逐帧、逐关节、root-relative 误差与同模式 clean 配对的有符号增量、正增量 AUC、峰值。恢复时间因容差尚未预先标定而留空。
- **阶段 B，可重跑体型/旋转分解。** `run/diagnose_fk_dense.py` 固定已保存的 v3 Gaussian clean 轨迹及输入 SHA，计算预测旋转 × 固定启动/逐帧预测/GT 体型，以及 GT 旋转 × 固定启动/GT 体型。世界 SMPL22 MPJPE（mm）：足球 **172.38 / 153.55 / 147.03 / 140.57 / 117.52**；Covid **261.24 / 224.21 / 231.02 / 116.04 / 105.79**。还报告 root、root-relative、PA、每关节 FK-dense、骨长、局部旋转和阶段值。Covid 的逐帧预测体型在该预测旋转下比 GT 体型误差更低，说明各因素不能按误差范数线性归因。GT 替换严格限于离线诊断，不进入模型/缓存/正式指标；旧 v3 使用**随机 E7 启动**。原始完整 JSON 见 `verification/issue3_fk_dense_soccer_gaussian_clean.json` 与 `issue3_fk_dense_covid_gaussian_clean.json`。
- **阶段 C，最小可微 FK 训练项。** `FixedShapeFK` 支持每样本 `[B,10]` 启动 β，冻结 SMPL-X 参数但保留预测旋转梯度；`physical_objective` 可加独立世界 SMPL22 FK 平方损失，沿用 0.1 m 缩放。单元测试验证不同样本 β 不串用、姿态梯度有限且非零、有限差分相符；真实 Version 1.0 SMPL-X 资产 2 样本反传得到姿态梯度 L1 **0.01916**，误差和梯度均有限。FK loss 拒绝 GT 启动/缺少来源标志的行。
- **阶段 D，真实 E7 到位后的公共启动和连续 FK 选模接口。** `run/cache_e7_bootstraps.py` 只读取各 take 干净前 20 帧，保存模型启动运动、reference、世界关节、β、地面估计和权重/统计/划分身份；在线回放重解码核验并复用。预测历史采集默认要求该缓存，旧 GT 启动必须显式 `--allow-gt-bootstrap` 且不能用于正式 FK。训练入口新增 `--fk-weight` 和 `--flow-selection closed_loop_fk`，每个候选 G 在固定干净开发 episode 上连续预测历史回放，以世界 SMPL22 MPJPE 选模。P 当前仍先做单步选择，不能声称其闭环收益；Q 仍未正式训练。
- **验证与限制。** `egorecover` 完整回归 **66 passed**；随机初始化的 1 步 P/G 训练入口烟测完成，仅验证接口。两条旧 v3 × 6 回放重新完成真实 SMPL-X 评估并生成 `verification/issue3_smplx_soccer.json`、`issue3_smplx_covid.json`。故障期相对同模式 clean 的漂移有符号增量（足球 Gaussian/History **+6.59/+5.37 mm**，Covid **+11.55/+7.09 mm**），与既有审阅结论一致；视觉冻结接近零，不能证明鲁棒性。
- 用户确认**没有训练好的 E7 Flow checkpoint**。因此公共启动缓存、实际 FK 训练、同预算 C0/C1 连续回放、可靠干净启动、P/Q 收益和独立测试结果均未执行；不能把接口或随机启动诊断写成方法可行性/精度证明。下一次必须先取得/训练可核实的 E7 Flow 权重，再按 `EGORECOVER.md` 的固定对照协议运行，并对最终测试另建独立划分。
- 15:41 交接信号新增第二个 `pilot` 数据集（完整验证集），原来只按 profile 唯一选择的读取器会报错。已改为按本轮固定 spec SHA256 精确选择旧 pilot，实测 `open_dataset()` 指向 `data/ee4d_mismatch_pilot_v0`；新增重复 profile 选择测试。两份 SMPL-X 故障配对报告由最终代码重生成，数值未变。完整回归最终 **66 passed**。
